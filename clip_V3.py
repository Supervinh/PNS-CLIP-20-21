...
def dat(ville, data, months):
    ...
    for month in months:
        ...
        if not (len(cities_v2)>1 or len(data_v2)>1):
            abscisses += [month]
...
while reponse =='yes':
    ...
    if not(len(cities_v2)>1 or len(data_v2)>1 or 'all' in data_v2):
        print("The resulting plot is:")
        graph(abscisse, ordonnee, dic_data_name[data_v2[0]], cities_v2[0])
    else:
